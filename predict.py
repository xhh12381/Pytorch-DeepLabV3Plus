...
def predict_img(net,
    ...
    img = img.unsqueeze(dim=0)  # 扩展维度：(c,h,w)->(1,c,h,w)
    ...
    with torch.no_grad():
        ...
        full_mask = tf(probs.cpu()).squeeze()  # 降维，将输入张量形状中的1去除并返回
...
def get_args():
    ...
    parser.add_argument('--model', '-m', default='./checkpoints/checkpoint_epoch90.pth', metavar='FILE',
...
if __name__ == '__main__':
    ...
    for i, filename in enumerate(imgs):
        ...
        if not args.no_save:
            ...
            save_path = os.path.join(test_result, (filename.split('\\')[1]).split('.')[0] + '_pred.png')
