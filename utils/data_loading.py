...
class BasicDataset(Dataset):
    ...
    def preprocess(pil_img, scale, is_mask):
        ...
        img_ndarray = np.asarray(pil_img).astype(np.uint8)  # PIL Image -> ndarray(h,w,c)或(h,w)
        ...
        # else:
        #     img_ndarray = cv2.dilate(img_ndarray, np.ones(shape=(5, 5), dtype=np.uint8))  # 对 mask 进行膨胀处理
